...
export function AssertionDetail({ assertion, index, data, atBrowsers }: { assertion: any; index: number; data: any; atBrowsers: ATBrowsers }) {
  return (
            ...
            return (
              <ResponsiveTable key={atType.type}>
                <table className="support-summary-table">
                  <caption>
                    {atType.title} support for &apos;{assertion.strength[atType.type]} {assertion.title}&apos;
                  </caption>
                  <colgroup span={1} />
                  ...
                  <tbody>
                    ...
                    {data.tests.map((linked_test: any) => {
                      const test_assertions = (linked_test.assertions ?? []).filter(
                        // eslint-disable-next-line @typescript-eslint/no-explicit-any
                        (obj: any) => obj.feature_id === data.id && obj.feature_assertion_id === assertion.id
                      );
                      // eslint-disable-next-line @typescript-eslint/no-explicit-any
                          ...
                          <th scope="row">
                            <Link to={`/tests/${makeSafe(linked_test.id)}`}>
                              ...
                              {test_assertion.applied_to && (
                                <span className="applied_to"> applied to: {test_assertion.applied_to_title}</span>
                              )}
                            </Link>
                          </th>
                          {coreAtForType.flatMap((at) =>
